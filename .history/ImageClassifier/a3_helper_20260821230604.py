...
from eecs498.data import cifar10
from eecs498.data import reset_seed, tensor_to_image
...
def get_CIFAR10_data(validation_ratio=0.02, flatten=False):
    ...
    """
    X_train, y_train, X_test, y_test = eecs498.data.cifar10()
    ...
    plt.imshow(eecs498.tensor_to_image(img))
    plt.axis("off")
    plt.show()
...
def svm_loss(x, y):
    ...
    margins = (x - correct_class_scores[:, None] + 1.0).clamp(min=0.0)
